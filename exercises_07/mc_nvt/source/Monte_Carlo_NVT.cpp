...
//update walker
  ...
  if (print_instant == true) {
    ofstream InstantPres, InstantEpot;

    InstantEpot.open("instant_epot.0",ios::app);
    InstantPres.open("instant_pres.0",ios::app);

    InstantEpot << walker[iv]/(double)npart + vtail << endl; //Potential energy
    InstantPres << rho * temp + (walker[iw] + ptail * (double)npart) / vol << endl; //Pressure
...
//g(r)
    ...
    for (int bin=0; bin<nbins; ++bin){
      ...
      err_gofr[bin]=Error(glob_av[igofr+bin],glob_av2[igofr+bin],iblk);
      r += bin_size;
    }
//final g(r)
    if (iblk == nblk){
      r = bin_size;
      for (int bin=0; bin<nbins; ++bin){
        Gave << setw(wd) << bin <<  setw(wd) << r << setw(wd) << glob_av[igofr+bin]/(double)iblk << setw(wd) << err_gofr[bin] << endl;
        r += bin_size;
      }
    }

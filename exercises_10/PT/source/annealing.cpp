...
Annealing :: Annealing(std::shared_ptr<Random> generator, std::vector<City> geometry, std::vector<double> rates, double b ){
...
void Annealing :: SetConfiguration(std::vector<int> conf){
  individual.SetGenotype(conf);
  individual.Update();
}
...
void Annealing :: Print(int rank,int istep){
    individual.Print(rank,istep);
}

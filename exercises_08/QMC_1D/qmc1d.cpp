...
has been either acquired from "input.in" by the readInput() function or
...
double variationalWaveFunction(double v)
{
  ...
  double mean = 0.794539;
  double sigma = 0.648469;
  return exp( -(v-mean)*(v-mean)/(sigma*sigma) ) + exp( -(v+mean)*(v+mean)/(sigma*sigma) ) + 2*exp( -(v*v+mean*mean)/(sigma*sigma) );
...
double variationalWaveFunction_second(double v)
{
  ...
  double mean = 0.794539;
  double sigma = 0.648469;
  double m = (v-mean)*(v-mean)/(sigma*sigma);
  double p = (v+mean)*(v+mean)/(sigma*sigma);
  return ( (m-1)*exp(-m/2)  + (p-1)*exp(-p/2) ) / (sigma*sigma);
